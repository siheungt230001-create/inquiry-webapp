...
// 2026-09-14 조혜성 학생 "무신 정권이 권문세족 성장을 저지했다면" 건 - 무신정권과
// 권문세족은 서로 다른 시기의 세력인데다, [읽기자료]는 무신정권기 가문이 권문세족
// 형성에 "참여"했다고 서술해 인과 방향도 가정과 반대다. 기존 (A-2)는 "같은 시점에
// 존재했는가"만 확인해서, 두 대상이 같은 시기였다면 방향이 뒤집힌 가정은 놓칠 수
// 있었다 - (A-3)에서 공존 여부와 별개로 인과 방향까지 명시적으로 확인하게 했다.
assert(prompt.includes("(A-3) 가정형(반사실적 가정) 질문의 공존·인과 방향 확인"), "프롬프트에 (A-3) 가정형 공존·인과 방향 확인 섹션 포함");
assert(prompt.includes("가정형 질문일 때는"), "프롬프트에 (A-3)가 가정형 질문에만 적용됨이 명시됨");
assert(prompt.includes("공존 확인:"), "프롬프트에 (A-3) 공존 확인 단계 포함");
assert(prompt.includes("인과 방향 확인:"), "프롬프트에 (A-3) 인과 방향 확인 단계 포함");
assert(
  prompt.includes("이 확인은 가정형 질문에만 적용한다"),
  "프롬프트에 (A-3)가 가정형 아닌 질문엔 적용 안 됨이 명시됨(오탐 방지)"
);
