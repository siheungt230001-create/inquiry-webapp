...
(A-3) 가정형(반사실적 가정) 질문의 공존·인과 방향 확인 (사실 정확성)
"~했다면", "~했으면", "~하지 않았다면", "~막았다면/저지했다면" 같은 반사실적
가정형 질문일 때는 위 (A-2)의 "같은 시점에 존재했는가"만으로 충분하지 않다 -
두 대상이 같은 시기에 존재했더라도, 가정이 전제하는 "인과관계의 방향" 자체가
[읽기자료]가 서술한 실제 시간 순서·인과 흐름과 반대일 수 있다. 가정형 질문에는
반드시 아래 두 가지를 순서대로 확인한다:
1. 공존 확인: 가정 속에 등장하는 주체·세력·사건(가정의 행위자 A, 그 영향을
   받는다고 전제된 대상·사건 B)이 실제로 같은 시기에 존재했거나, A가 B에
   영향을 줄 수 있는 시점에 아직 힘을 갖고 있었는가? (예: A라는 세력이 B라는
   사건이 벌어지기 전에 이미 다른 세력에 흡수되어 사라졌다면, A는 B 시점에
   더 이상 행위자로서 존재하지 않으므로 "A가 B를 막았다면"이라는 가정 자체가
   성립하지 않는다.)
2. 인과 방향 확인: [읽기자료]가 서술한 실제 관계가 가정과 반대 방향은 아닌가?
   (예: [읽기자료]에 "A 세력 출신 가문이 B 세력 형성에 참여했다"처럼 A가 B로
   이어지는/흡수되는 관계로 서술돼 있는데, 질문이 "A가 B의 성장을 막았다면"처럼
   반대로 대립·저지 관계를 가정하면 인과 방향이 뒤집힌 것이다.)
   위 1·2 중 하나라도 해당하면 anachronism_detected를 true로 놓고 사실
   정확성을 0점으로 준다(위 (A-2)와 같은 처리).
- 피드백은 위 (A-2)와 같은 확인 질문 톤을 쓰되, 가정형 질문에서는 무엇이
  왜 시간·인과상 안 맞는지 조금 더 구체적으로 짚어준다: "이 가정은 시간
  순서상 성립하기 어려워 보여요. OO는 △△ 시기에 있었던 일이고, XX는 그보다
  뒤에 일어난 일이라 서로 영향을 주고받기 어려웠을 것 같아요. [읽기자료]에서
  두 사건이 어떤 순서로 이어지는지 다시 확인해볼까요?"처럼, 두 대상의 선후
  관계를 짚어주는 확인 질문 형태로 안내한다(정답이나 실제 역사적 결과는
  여전히 알려주지 않는다).
- 이 확인은 가정형 질문에만 적용한다. 가정형이 아닌 일반 서술형 질문은 계속
  (A-2)의 기준만 따른다.
...
위 [시대착오 확인](A-2) 또는 (A-3, 가정형 질문의 공존·인과 방향 확인) 기준대로
판정한 결과를 boolean으로 넣는다. 둘 중 하나라도 걸리면 true(그리고
criteria_scores.fact_accuracy는 0), 둘 다 아니면 false다. 확신이 서지 않으면
false로 둔다(과잉 적용 금지).
