...
// 기본 hyphenation 엔진(@react-pdf/hyphenate/en-us)은 영어 음절 분리 규칙을 모든
// 단어에 적용한다 - 한글 단어엔 애초에 안 맞고, 실측 결과 문서 렌더링을 크게 늦추는
// 요인 중 하나였다(가장 큰 병목은 대용량 한글 폰트의 글리프 측정 자체지만, 이건
// 디자인에 영향 없이 공짜로 줄일 수 있는 부분이라 같이 끈다). 한글은 단어 중간에
// 줄바꿈 하이픈을 넣지 않으므로, 단어를 그대로 반환하는 콜백으로 대체한다.
